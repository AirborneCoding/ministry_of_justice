...
const FilterButtons: React.FC<FilterButtonsProps> = ({ selectedChart, handleChartToggle }) => {
    return (
        <div className="flex gap-6">
            <button
                className={`text-myBlue flex space-x-2 items-center ${selectedChart === 'bar' ? 'bg-grona' : 'bg-gray-200'
                    } hover:bg-grona hover:text-white active:bg-yellow-600 active:text-white rounded-md py-2 px-4`}
            ...
            <button
                className={` text-myBlue flex space-x-3 items-center ${selectedChart === 'area' ? 'bg-grona' : 'bg-gray-200'
                    } hover:bg-grona hover:text-white active:bg-yellow-600 active:text-white rounded-md py-2 px-4`}
